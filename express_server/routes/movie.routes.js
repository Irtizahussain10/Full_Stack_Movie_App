...
router.get('/', (req, res) => {
  res
    .sendFile(path.join('C:/Users/SAAD COMMUNICATION/OneDrive/Desktop/Software Side/WebApps/mflix/express_server', '/index.html'));
});
  ...
  if (req.params.PageNumber < 0) {
    ...
    try {
      ...
      let response = await moviesController
        .getMovies(Number(req.params.PageNumber));
...
router.get('/searchMoviesByCast/:Actor', async (req, res) => {
  ...
  try {
    ...
    let response = await moviesController
      .getMoviesByActor(req.params.Actor);
    res.send(response);
...
router.get('/searchMoviesByCountry/:Country/:Page', async (req, res) => {
  ...
  try {
    ...
    let response = await moviesController
      .getMoviesByCountry(
        Number(req.params.Page), req.params.Country
      );
    res.send(response);
...
router.get('/textSearch/:text/:page', async (req, res) => {
  ...
  try {
    ...
    let response = await moviesController
      .getMoviesByText(
        req.params.text, req.params.page
      );
    res.send(response);
